...
/**/ int main(int argc, char **argv)
{
  ...
  FILE *fp_sos_size, *fp_cl_kept;
  FILE *fp_cl_generated;
  FILE *fp_for_sub;

  int write_counter = 0;
  
  remove("sos_size.txt"); 
  fp_sos_size = fopen("sos_size.txt", "a");

  remove("cl_kept.txt"); 
  fp_cl_kept = fopen("cl_kept.txt", "a");

  remove("cl_generated.txt"); 
  fp_cl_generated = fopen("cl_generated.txt", "a");

  remove("for_sub.txt"); 
  fp_for_sub = fopen("for_sub.txt", "a");
  ...
  if (errors != 0) {
	...
	log_for_x_show(xlog_fp);
      ...
      fprintf(fp_sos_size, "%ld \n", Stats[SOS_SIZE]);
      fprintf(fp_cl_kept, "%ld \n", Stats[CL_KEPT]);
      fprintf(fp_cl_generated, "%ld \n", Stats[CL_GENERATED]);

      //if(write_counter % 100 == 0)
      fprintf(fp_for_sub, "%ld \n", Stats[CL_FOR_SUB]);
